...
Định dạng dữ liệu: Fixed-point Q3.13 (16-bit).
  1 bit dấu (Sign).
  2 bit phần nguyên (Integer).
  13 bit phần thập phân (Fractional).
...
Kiến trúc: RTL Structural (Tách biệt Controller và Datapath).
3. Cấu trúc thư mục (File Structure)
Thứ tự biên dịch quan trọng như sau:
  Mylib.vhd: Package chứa khai báo các Component (Datapath, Controller, Reg_n).
  ...
  Datapath.vhd: Khối xử lý dữ liệu. Chứa bảng LUT, bộ cộng/trừ/dịch bit và các thanh ghi trạng thái X, Y, Z.
    Lưu ý: Ngõ ra exp được gán trực tiếp để đảm bảo không bị trễ nhịp khi tín hiệu done lên 1.
...
Hệ thống hoạt động dựa trên thuật toán CORDIC Hyperbolic:
  ...
  Tính toán (Calculation - 13 vòng):Tại mỗi bước $i$, tùy thuộc vào dấu của $Z$:
...
Bước 3: Chạy Testbench
  ...
  Run mô phỏng trong khoảng 1 us (1000 ns).
  Quan sát dạng sóng (Waveform).
Bước 4: Kiểm tra kết quả
Để dễ quan sát, hãy chuyển định dạng hiển thị (Radix) của t_in và exp_out sang Decimal (Signed).
Bảng kết quả kỳ vọng (Q3.13):
Test CaseInput t (Float)Input t (Decimal)Output Kỳ vọng (et)Output exp (Decimal)Case 1$1.0$8192$2.718$$\approx 22268$Case 2$0.5$4096$1.648$$\approx 13506$Case 3$-0.5$-4096$0.606$$\approx 4968$Case 4$0.0$0$1.0$$8192$
6. Lưu ý quan trọng
